...
namespace Wasp.Extensions
{
    ...
    public static class Verification
    {
        ...
        public static TSelectable VerifySelected<TSelectable>(this TSelectable selectable, bool selected) where TSelectable : ISelectable
        {
            if (selectable.Selected != selected)
            {
                throw CreateVerificationException(selectable,
                    String.Format("Selection verification failed. Expected: {0}, Actual: {1}.", selected, selectable.Selected));
            }
        ...
        public static THasText VerifyTextMismatch<THasText>(this THasText hasText, string text) where THasText : IHasText
        {
            ...
            {
                throw CreateVerificationException(hasText,
                    String.Format("Text mismatch verification failed. Unexpected:  {0}, Actual:  {1}.", text, hasText.Text));
            }
        ...
        public static void VerifyClasses(this IWebElement element, IEnumerable<string> expectedClasses)
        {
            ...
            if (missingClasses.Any())
            {
                throw CreateVerificationException(element,
                    String.Format("Block is missing the following expected classes: {0}", String.Join(", ", missingClasses)));
            }
        }
        ...
        private static VerificationException CreateVerificationException(object item, string message, Exception innerException = null)
        {
            ...
            if (hasSession != null)
            {
                ...
                if (session != null)
                {
                    ...
                    {
                        var method = CallStack.GetFirstNonWaspMethod();
